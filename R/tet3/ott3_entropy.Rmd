...
gene.omp <- makeImage("omp_hmc_120424_rpkm", "gene_whole_W200N50F50_chr",  data_type="rpkm/mean", image=FALSE)
gene.ott3 <- makeImage("ott3_1_hmc_rpkm", "gene_whole_W200N50F50_chr",  data_type="rpkm/mean", image=FALSE)
...
gene.ott3.omp.rna.dis <- foreach(x=isplitCols(gene.ott3.omp.rna, chunkSize=1), .combine="cbind")  %do% discretize(x, breaks)
...
tab <- alply(gene.ott3.omp.rna.dis, 2, function(x) table(factor(x, levels=1:length(breaks.dna)), ott3.omp.rna))
```
...
Plot
```{r gene_ott3_omp_hmc_log2_vs_ott3_omp_rmrna_mi, dev=c("png", "pdf"), fig.width=4, fig.height=2}
theme_set(theme_bw())
gg <- ggplot(data.frame(x=1:150, y=mi.val), aes(x,y))
gg <- gg + geom_point() + stat_smooth(method="loess", se=F, span=.07) + geom_vline(xintercept=c(51, 100), linetype=2)
gg <- gg + scale_x_discrete(breaks=c(1, 51, 100, 150), labels=c("-10 kb", "TSS", "TES", "+10kb")) + xlab("") + ylab("Mutual information")
gg <- gg + theme(axis.text.x=element_text(size=16), axis.text.y=element_text(size=16), axis.title.y=element_text(size=16))
gg
```
...
### TSS data
```{r}
tss.omp <- makeImage("omp_hmc_120424_rpkm", "refGene_noRandom_order_outsides2_tss_W25F200_chr", data_type="rpkm/mean", image=F)
tss.ott3 <- makeImage("o.tt3.1_hmc_rpkm", "refGene_noRandom_order_outsides2_tss_W25F200_chr", data_type="rpkm/mean", image=F)
...
#breaks.dna <- seq(min(tss.ott3.omp.rna), max(tss.ott3.omp.rna), length.out=75)
tss.ott3.omp.rna.dis <- foreach(x=isplitCols(tss.ott3.omp.rna, chunkSize=1), .combine="cbind")  %do% discretize(x, breaks.dna)
...
```{r}
ott3.omp.rna <- factor(rna.1log2.dna.dis, levels=1:length(breaks.rna))
tab <- alply(tss.ott3.omp.rna.dis, 2, function(x) table(factor(x, levels=1:length(breaks.dna)), ott3.omp.rna))
```
...
Plot
```{r tss_ott3_omp_hmc_log2_vs_ott3_omp_rmrna_mi, dev=c("png", "pdf"), fig.width=4, fig.height=2}
library(ggplot2)
theme_set(theme_bw())
gg <- ggplot(data.frame(x=1:400, y=mi.val), aes(x,y))
gg <- gg + geom_point() + stat_smooth(method="loess", se=F, span=.07) + geom_vline(xintercept=201, linetype=2)
gg <- gg + scale_x_discrete(breaks=c(1, 201, 400), labels=c("-5 kb", "TSS", "+5kb")) + xlab("") + ylab("Mutual information")
gg <- gg + theme(axis.text.x=element_text(size=16), axis.text.y=element_text(size=16), axis.title.y=element_text(size=16))
gg
```
...
cells.rna.1log2 <- cbind(cells.rna.1log2, data.frame(rmrna.omp=rna.1log2[m,2], rmrna.ott3=rna.1log2[m,3]))
cells.rna.1log2 <- na.omit(cells.rna.1log2)

breaks.rna <- hist(as.matrix(cells.rna.1log2), breaks="FD", plot=F)$breaks
cells.rna.1log2.dis <- foreach(x=isplitCols(as.matrix(cells.rna.1log2), chunkSize=1), .combine="cbind")  %do% discretize(x, breaks.rna)

colnames(cells.rna.1log2.dis) <- colnames(cells.rna.1log2)
co <- combn(colnames(cells.rna.1log2.n), 2)
tab <- alply(co, 2, function(x) table(factor(cells.rna.1log2.n.dis[,x[1]], levels=1:length(breaks.rna)),factor(cells.rna.1log2.n.dis[,x[2]], levels=1:length(breaks.rna))))
mi <- lapply(tab, mi.shrink, verbose=F)
mi.val <- unlist(lapply(mi, function(x) x[1]))
