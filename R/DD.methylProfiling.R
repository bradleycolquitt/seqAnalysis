...
MEDIPS.methylProfiling=function (data1 = NULL, data2 = NULL, input = NULL, ROI_file = NULL, frame_size = NULL, math = mean, step = NULL, select = 2, chr = NULL, transf=T)
{
    ...
    dd1.data <- dd1.$genome.data
    dd1.raw <- dd1.data$raw/(sum(dd1.data$raw)/1E6)
    ...
    if(!is.null(data2)){
        ...
        dd2.data <- dd2$genome.data
        dd2.raw <- dd2.data$raw/(sum(dd2.data$raw)/1E6)
    ...
    if(!is.null(ROI_file) | !is.null(chr)) {
        ...
        if (!is.null(indices)) {
            ...
            dd1.data <- dd1.data[indices,] 
            dd1.chr_names <- unique(dd1.data[,'chr'])
