...
# from Storeys qvalue package, with gui stuff removed
...
chrNames <- paste("chr",c(1:19,"X","Y"),sep = "")
...
numVecToChr <- function(num.vec) {
  chr.vec <- vector(length=length(num.vec), mode="character")
  for (num in 1:21) {
    chr.vec[num.vec == num] <- numToChr(num)
  }
  return(chr.vec)
}
strandToNum <- function(strand.vec) {
  bin.vec <- rep(0,times=length(strand.vec))
  bin.vec[strand.vec == "+"] <- 1
  return(bin.vec)
}

numToStrand <- function(bin.vec) {
  strand.vec <- rep("-",times=length(bin.vec))
  strand.vec[bin.vec] <- "+"
  return(strand.vec)
}

geneToNum <- function(gene.vec) {
  num.vec <- rep(NA, times=length(gene.vec))
  coding <- read.delim("/home/user/lib/gene_integer_codes.txt", header=F, colClasses=c("character","integer"))
  index <- match(gene.vec,coding[,1])
  num.vec <- coding[index,2] 
  return(num.vec)
}

numToGene <- function(num.vec) {
  gene.vec <- rep(NA,times=length(num.vec))
  coding <- read.delim("/home/user/lib/gene_integer_codes.txt", header=F, colClasses=c("character","integer"))
  index <- match(num.vec,coding[,2])
  gene.vec <- coding[index,1]
  return(gene.vec)
}
matcher<-function(one,two){
  m<-data.frame(match(two,one))
  m[,2]<-c(1:length(m[,1]))
  m<-na.omit(m)
  m
}

##vals is triple with name, group, value columns
splitToMatrix <- function(data, vals) {
  sp <- split(data[, vals[3]], data[,vals[2]])
  mat <- matrix(nrow=length(sp[[1]]), ncol=length(sp))
  for (i in 1:length(sp)) {
    mat[,i] <- sp[[i]]
  }
  names.u <- unique(data[,vals[1]])
  rownames(mat) <- names.u
  return(mat)
}
