setClass(Class = 'AtoM',
         representation = representation(
           ...
           genome_CF='numeric',         # CouplingVector
           genome_raw='numeric',
           genome_norm='numeric',
...
setMethod('number_regions','AtoM', function(object) object@number_regions)

AnnoSet.MPinterface <- function(annoset, reads) {
  ## Make dummy class (interface) for feed into profile.c
  atom <- new('AtoM', name=annoset@name,
              chr_names=annoset@chr_names,
              genome_chr=annoset@set_chr,
              genome_pos=annoset@set_pos,
              chr_lengths=annoset@chr_lengths,
              bin_size=annoset@bin_size,
              genome_raw=reads@raw,
              genome_norm=reads@norm,
              genome_CF=annoset@coupling_vector@setCoup)
              number_regions=reads@number_regions
  return(atom)
}

AnnoSet.MP.all <- function(annoset) {
  out.path <- paste("~/analysis/mprofiles", annoset@name, sep="/")
  profiles <- foreach(reads=annoset@read_data) %dopar% {
    atom <- AnnoSet.MPinterface(annoset, reads)
    profile <- MEDIPS.methylProfiling(data1=atom, frame_size=annoset@bin_size)
    profile <- cbind(profile, names=annoset@set_names, group=annoset@set_group)
    write.table(profile, file=paste(out.path, reads@name, sep="/"), quote=FALSE, sep="\t",
...
AnnoSet.MP <- function(annoset) {
  atom <- AnnoSet.MPinterface(annoset, annoset@read_data[[1]])
  profile <- MEDIPS.methylProfiling(data1=atom, frame_size=annoset@bin_size)
  profile <- cbind(profile, names=annoset@set_names, group=annoset@set_group)
  write.table(profile, file=paste(out.path, reads@name, sep="/"), quote=FALSE, sep="\t",
                row.names=FALSE, col.names=FALSE)
