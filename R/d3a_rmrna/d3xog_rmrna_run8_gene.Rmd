...
gene.read.fpkm.1log2 <- numcolwise(onelog2)(gene.read.fpkm)
gene.read.fpkm.1log2$id <- gene.read.fpkm$tracking_id
cor(gene.read.fpkm)
```
...
#### Ordered by WT RNA, just D3A
```{r}
gene.read.fpkm.d3a <- gene.read.fpkm[,c("q1_2", "q2_0", "q3_0", "q3_1")]
gene.read.fpkm.d3a <- gene.read.fpkm.d3a[order(gene.read.fpkm.d3a[,1]),]
gene.read.fpkm.d3a.sub <- cbind(gene.read.fpkm.d3a, pairwise(gene.read.fpkm.d3a[,4:1], subtract))
```

```{r}
gene.read.fpkm.d3a.sub.c100 <- chunkMatrix(gene.read.fpkm.d3a.sub, 100, mean)
head(gene.read.fpkm.d3a.sub.c100)
gene.read.fpkm.d3a.sub.c100.m <- melt(gene.read.fpkm.d3a.sub.c100, id.vars="index")
```

```{r}
gg <- ggplot(gene.read.fpkm.d3a.sub.c100.m[gene.read.fpkm.d3a.sub.c100.m$variable %in% c("q1_2", "q3_0", "q3_1"),], aes(index, value, color=variable))
gg + geom_point()
```
...
### MAplots
```{r d3xog_wt_het_ko_rmrna_run8_wt_ko_maplots, fig.width=8, fig.height=4, dev=c("png", "pdf")}
