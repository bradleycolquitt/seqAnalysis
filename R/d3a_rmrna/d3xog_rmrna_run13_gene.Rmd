...
gene.fpkm.mat.c100 <- chunkMatrix(gene.fpkm.mat.1log2, 100, median)
gene.fpkm.mat.c100.m <- melt(gene.fpkm.mat.c100, id.vars="index")
```

### MAplots
```{r d3xog_wt_het_ko_rmrna_run13_wt_ko_maplots, fig.width=8, fig.height=4, dev=c("png", "pdf")}
rna.gg <- ggplot(gene.fpkm.mat, aes(q1_FPKM, q3.q1)) + geom_point(alpha=I(1/10)) + geom_density2d(breaks=seq(0, .5,.025)) + labs(x="WT log2(FPKM + 1)", y="KO - WT log2(FPKM + 1)") + geom_hline(yintercept=0, color="red")
#rna.gg
rna2.gg <- ggplot(gene.fpkm.mat, aes(q3_FPKM, q3.q1)) + geom_point(alpha=I(1/10)) + geom_density2d(breaks=seq(0, .5,.025)) + labs(x="KO log2(FPKM + 1)", y="KO - WT log2(FPKM + 1)") + geom_hline(yintercept=0, color="red")
...
### Ordered by WT RNA
```{r}
gene.fpkm.mat <- gene.fpkm.mat[order(gene.fpkm.mat[,1]),]
...
#### Only WT and KO
```{r}
gg <- ggplot(gene.fpkm.mat.c100.m[gene.fpkm.mat.c100.m$variable %in% c("q1_FPKM", "q3_FPKM", "q3.q1"),], aes(index, value, color=variable))
gg + geom_point()
```
...
```{r}
omp <- read.delim("~/lib/annotations_hires/refGene_noRandom_order_outsides2_tss_W25F200_omp_ngn_icam_mrna_ucsc_fc1_fpkm1_omp_vs_ngn_icam.bed", header=F)
on <- read.delim("~/lib/annotations_hires/refGene_noRandom_order_outsides2_tss_W25F200_omp_ngn_icam_mrna_ucsc_fc1_fpkm1_omp_ngn_vs_icam.bed", header=F)
ngn <- read.delim("~/lib/annotations_hires/refGene_noRandom_order_outsides2_tss_W25F200_omp_ngn_icam_mrna_ucsc_fc1_fpkm1_ngn_vs_omp_icam.bed", header=F)
ni <- read.delim("~/lib/annotations_hires/refGene_noRandom_order_outsides2_tss_W25F200_omp_ngn_icam_mrna_ucsc_fc1_fpkm1_icam_ngn_vs_omp.bed", header=F)
icam <- read.delim("~/lib/annotations_hires/refGene_noRandom_order_outsides2_tss_W25F200_omp_ngn_icam_mrna_ucsc_fc1_fpkm1_icam_vs_omp_ngn.bed", header=F)

gene.fpkm.mat.1log2.m$cell.expr <- "none"
gene.fpkm.mat.1log2.m$cell.expr[gene.fpkm.mat.1log2.m$id %in% omp[,4]] <- "omp"
gene.fpkm.mat.1log2.m$cell.expr[gene.fpkm.mat.1log2.m$id %in% on[,4]] <- "on"
gene.fpkm.mat.1log2.m$cell.expr[gene.fpkm.mat.1log2.m$id %in% ngn[,4]] <- "ngn"
gene.fpkm.mat.1log2.m$cell.expr[gene.fpkm.mat.1log2.m$id %in% ni[,4]] <- "ni"
gene.fpkm.mat.1log2.m$cell.expr[gene.fpkm.mat.1log2.m$id %in% icam[,4]] <- "icam"
```

```{r}
gg <- ggplot(gene.fpkm.mat.1log2.m, aes(variable, value))
gg + geom_boxplot() + facet_wrap(~cell.expr)
```
