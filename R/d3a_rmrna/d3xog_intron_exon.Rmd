...
#intron <- makeFeatureMatrix2("Refgene_intron_map1_chr", "d3xog_rmrna_rmdup", data_type="bam/sum", include_score=T)
#intron.bed <- read.delim("~/lib/features_general/Refgene_intron_map1", header=F)
```

```{r}
intron_totals <- lapply(intron, function(x) sum(x[,7]))
intron_norm <- lapply(1:4, function(x) {
  intron[[x]][,7] <- intron[[x]][,7] / intron_totals[[x]]
  return(intron[[x]])
})
...
#exon <- makeFeatureMatrix2("Refgene_exon_map1_chr", "d3xog_rmrna_rmdup", data_type="bam/sum", include_score=T)
#exon.bed <- read.delim("~/lib/features_general/Refgene_exon_map1", header=F)
```

```{r}
exon_totals <- lapply(exon, function(x) sum(x[,7]))
exon_norm <- lapply(1:4, function(x) {
  exon[[x]][,7] <- exon[[x]][,7] / exon_totals[[x]]
  return(exon[[x]])
})
...
```{r}
palette <- 
gg <- ggplot(comb.df, aes(wt, ko1, color=hmc.ko.wt))
gg + geom_point() + color_scale_continuous()
```
