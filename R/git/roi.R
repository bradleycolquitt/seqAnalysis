...
loadROI <- function(fname, anno = NULL ) {
  colClasses <- c("character","integer","integer")
  if (!is.null(anno)) {
    if (anno == "gene") colClasses <- c("character", "integer", "integer", "character", "integer", "character")
    if (anno == "other") colClasses <- c("character", "interger", "integer", "integer", "integer", "character")
  }
  roi <- read.table(fname, sep='\t', header=FALSE, colClasses = colClasses)
  roi[,1] <- sapply(roi[,1], chrToNum)
  if (!is.null(anno)) {
    if (anno == "gene") roi[,4] <- sapply(roi[,4], geneToNum)
    roi[,6] <- sapply(roi[,6], strandToNum)
  }  
