...
def subtract_w_floor(one, two):
    out_array = np.zeros(len(one))
    # Convert array to list. Substantial speedup.
    one = list(one)
    two = list(two)
    for ind in range(len(one)):
        result = one[ind] - two[ind]
        if result > 0: out_array[ind] = result
    return out_array
    
def run(one_track, two_track, out):
    ...
    for chr in one_track._f_iterNodes():
        ...
        out_track_chr = subtract_w_floor(one_track_chr, two_track_chr)
