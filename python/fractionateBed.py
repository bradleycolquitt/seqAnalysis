...
def main(argv):
  ...
  outfile = open(args.input + "_W" + args.window + "_fraction", 'w')
  ...
  for line in infile:
    ...
    window_set = bed_length / window
    if window_set == 1: continue
    r = range(window_set)
  #    if strand == "+":
  ...
  #      r = xrange(flanks)
    ...
    for index in r:
      out = (sline[0] + "\t" + str(start) + "\t" + str(end) + "\t" + \
                name + "\t" + str(round(float(index)/(window_set-1), 3)) + "\t" + strand + "\n")
