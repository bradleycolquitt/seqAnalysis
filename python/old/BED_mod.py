...
class BED7:
    ...
    def __init__(self, chrom, start, end, name, score, strand,reads):
        self.chrom = chrom;
        self.start = start;
        self.end = end;
        self.name = name;
    ...
    def __set__(self, chrom, start, end, name, score, strand,reads):
        self.chrom = chrom;
        self.start = start;
        self.end = end;
        self.name = name;
    ...
    def getCoord(self):
        outstring = self.chrom + "\t" + str(self.start) + "\t" + \
                    str(self.end) + "\t" + self.name + "\t" + \
                    str(self.score) + "\t" + self.strand + "\t" + str(self.reads);
        try:
            return outstring;
        except:
            sys.stderr.write("No coord information for %s\n" % self)
...
class BED:
    ...
    def __init__(self, species="hg18", file=None, bed_type="BED3", n="int", val_threshold=0):
            ...
            if re.match(bed_type, "BED3"):
                    ...
                    for line in infile:
                        ...
                        if not re.match("track", line):
                            ...
                            if len(sline) == 3:
                                sys.stderr.write("Can't make bed_graph with only \3 elements")
                                raise bedError 
                            elif len(sline) == 4:
                                #if atof(sline[3]) >= val_threshold:
                                if n == "name":
                                    #print sline[0]
                                    bed = BED_GRAPH(sline[0], atoi(sline[1]), atoi(sline[2]), str(sline[3]));
                                    self.bed_vals[sline[0]].append(bed);	
                                else:
                ...
                for line in infile:
                    ...
                    if not re.match("track", line):
                        ...
                        if len(sline) == 6:
                            #if atof(sline[4]) >= val_threshold:
                            if type(sline[4]) == type("x"):
                                bed = BED6(sline[0], atoi(sline[1]), atoi(sline[2]), sline[3], sline[4], sline[5]);
                            else:        
                           	bed = BED6(sline[0], atoi(sline[1]), atoi(sline[2]), sline[3], atof(sline[4]), sline[5]);
                            self.bed_vals[sline[0]].append(bed);

            elif re.match(bed_type, "BED7"):
                print "here"
                """ If want BED7 element """
                infile = open(file);
                for line in infile:
                    """ check to make sure not a header line """
                    if not re.match("track", line):
                        ...
                        if len(sline) == 6:
                            ...
                            #print "here2"
                            if type(sline[4]) == type("x"):
                                bed = BED7(sline[0], atoi(sline[1]), atoi(sline[2]), sline[3], sline[4], sline[5],0);
                            else:
                                bed = BED7(sline[0], atoi(sline[1]), atoi(sline[2]), sline[3], atof(sline[4]), sline[5],0);
                            self.bed_vals[sline[0]].append(bed);
