...
class tab:
    def __init__(self, feature, date, sample, window, fun):
        ...
        self.date = date
        self.sample_path = "/".join([SAMPLE_PATH, date, sample, window])
        self.sample = sample
        self.window = window
        self.fun = fun
        self.out_path = "/".join([OUT_PATH, os.path.basename(sample), os.path.basename(feature)])
    ...
    def run(self):
        ...
        for chr_tbp in chrs_tbp:
            ...
            self.tab_core(feature_data, sample_data, feature_out, self.fun)
            feature_data.close()
            sample_data.close()
            feature_out.close()    
        self.file_combine(tmp_path)

    def tab_core(self, feature_data, sample_data, feature_out, fun):
    ...
    def file_combine(self, tmp_path):
        feature = self.feature
        sample = self.sample
        out_prefix = OUT_PATH + "/" + self.date + "/" + os.path.basename(sample)
        print out_prefix
...
def main(argv):
    ...
    parser.add_argument('-s', dest='sample', required=False)
    ...
    if args.sample_set:
        ...
        if args.feature_set:
            features = [f for f in os.listdir(FEATURE_PATH) if re.search("chr", f)]
        samples = [args.sample]
    ...
    obj = tab(args.feature, args.date, args.sample, args.window, args.fun)
